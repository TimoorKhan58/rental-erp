...
export function UserEditPage({ userId }: UserEditPageProps) {
  ...
  const { canUpdate, isLoading: permissionsLoading } = useIdentityPermissions();
  const { data: currentProfile } = useCurrentIdentityProfile(canUpdate);
  ...
  if (!canUpdate) {
    return (
      <PageContainer>
        <AccessDeniedState description="You do not have permission to edit users." />
      </PageContainer>
    );
  }
