...
export function useUserListParams() {
  ...
  const params = useMemo<ListUsersParams>(() => {
    ...
    return {
      ...
      sortOrder: (searchParams.get("sortOrder") as ListUsersParams["sortOrder"]) ?? "desc",
  ...
  return {
    ...
    setRoleFilter: (role: UserRole | undefined) => updateParams({ role, page: DEFAULT_PAGE }),
    setSorting: (sortBy: UserSortField, sortOrder: ListUsersParams["sortOrder"] = "asc") =>
