...
import { getWarehouseTableColumns } from "./warehouse-table-columns";
...
type WarehouseListTableProps = {
  onCreateClick?: () => void;
};

export function WarehouseListTable({ onCreateClick }: WarehouseListTableProps) {
  const queryClient = useQueryClient();
  const { params, localSearch, setLocalSearch, setSearch, setPage, setStatusFilter, setSorting } =
    useWarehouseListParams();
  const { canCreate, canUpdate, canDelete } = useWarehousePermissions();
  const { data, isLoading, isError, error, refetch, isFetching } = useWarehouses(params);

  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [deleteTarget, setDeleteTarget] = useState<WarehouseResponse | null>(null);
  const [statusTarget, setStatusTarget] = useState<WarehouseResponse | null>(null);
  ...
  if (isError) {
    return (
      <div
        ...
        role="alert"
      >
        <p className="text-sm font-medium">Failed to load warehouses</p>
  ...
  return (
    <>
      <DataTableShell
            ...
            action={
              canCreate ? (
                onCreateClick ? (
                  ...
                  <Link href={ROUTES.warehousesNew}>
                    <EmptyStateActionButton>Create warehouse</EmptyStateActionButton>
                  </Link>
                )
              ) : undefined
            }
        ...
        loadingState={<LoadingState label="Loading warehouses..." />}
        pagination={
          data?.meta ? (
