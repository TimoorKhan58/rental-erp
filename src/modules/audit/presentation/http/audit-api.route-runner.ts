...
import type { AuditApplicationServices } from "@/modules/audit/application/services/audit-application-services.interface";

export interface AuditApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => AuditApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: AuditApplicationServices,
  ) => Promise<T>;
}

export async function runAuditApiRoute<T>(
  options: AuditApiRouteOptions<T>,
