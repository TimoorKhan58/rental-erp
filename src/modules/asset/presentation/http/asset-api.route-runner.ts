...
export interface AssetApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => AssetApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: AssetApplicationServices,
  ) => Promise<T>;
}

export async function runAssetApiRoute<T>(
  options: AssetApiRouteOptions<T>,
