...
export interface DispatchApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => DispatchApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: DispatchApplicationServices,
  ) => Promise<T>;
}

export async function runDispatchApiRoute<T>(
  options: DispatchApiRouteOptions<T>,
