...
export interface MaintenanceApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => MaintenanceApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: MaintenanceApplicationServices,
  ) => Promise<T>;
}

export async function runMaintenanceApiRoute<T>(
  options: MaintenanceApiRouteOptions<T>,
