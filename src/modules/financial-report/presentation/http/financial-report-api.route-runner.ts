...
export interface FinancialReportApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => FinancialReportApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: FinancialReportApplicationServices,
  ) => Promise<T>;
}

export async function runFinancialReportApiRoute<T>(
  options: FinancialReportApiRouteOptions<T>,
