...
export interface PaymentApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => PaymentApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: PaymentApplicationServices,
  ) => Promise<T>;
}

export async function runPaymentApiRoute<T>(
  options: PaymentApiRouteOptions<T>,
