...
import type { CategoryApplicationServices } from "@/modules/expense/application/services/category-application-services.interface";
...
export interface ExpenseCategoryApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => CategoryApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: CategoryApplicationServices,
  ) => Promise<T>;
}

export async function runExpenseCategoryApiRoute<T>(
  options: ExpenseCategoryApiRouteOptions<T>,
