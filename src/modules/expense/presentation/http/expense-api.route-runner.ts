...
import type { ExpenseApplicationServices } from "@/modules/expense/application/services/expense-application-services.interface";
...
export interface ExpenseApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => ExpenseApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: ExpenseApplicationServices,
  ) => Promise<T>;
}

export async function runExpenseApiRoute<T>(
  options: ExpenseApiRouteOptions<T>,
