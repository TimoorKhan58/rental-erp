...
export interface SupplierApiRouteOptions<T> {
  ...
  resolveServices: (ctx: ExecutionContext) => SupplierApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: SupplierApplicationServices,
  ) => Promise<T>;
}

export async function runSupplierApiRoute<T>(
  options: SupplierApiRouteOptions<T>,
