...
} from "@/modules/catalog/application/services/catalog-application-services.interface";
...
export interface CatalogApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  ...
  handler: (
    ...
    services: CatalogApplicationServices,
  ) => Promise<T>;
}

export async function runCatalogApiRoute<T>(
  options: CatalogApiRouteOptions<T>,
