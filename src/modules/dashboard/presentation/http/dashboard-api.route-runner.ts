...
export interface DashboardApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => DashboardApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: DashboardApplicationServices,
  ) => Promise<T>;
}

export async function runDashboardApiRoute<T>(
  options: DashboardApiRouteOptions<T>,
