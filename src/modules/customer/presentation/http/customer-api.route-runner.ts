...
import type { CustomerApplicationServices } from "@/modules/customer/application/services/customer-application-services.interface";
...
export interface CustomerApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => CustomerApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: CustomerApplicationServices,
  ) => Promise<T>;
}

export async function runCustomerApiRoute<T>(
  options: CustomerApiRouteOptions<T>,
