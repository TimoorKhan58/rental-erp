...
export interface PurchaseOrderApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => PurchaseOrderApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: PurchaseOrderApplicationServices,
  ) => Promise<T>;
}

export async function runPurchaseOrderApiRoute<T>(
  options: PurchaseOrderApiRouteOptions<T>,
