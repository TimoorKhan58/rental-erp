...
export interface IdentityApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  ...
  handler: (
    ...
    services: IdentityApplicationServices,
  ) => Promise<T>;
}

export async function runIdentityApiRoute<T>(
  options: IdentityApiRouteOptions<T>,
