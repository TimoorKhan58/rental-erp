...
export interface RentalInvoiceApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => RentalInvoiceApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: RentalInvoiceApplicationServices,
  ) => Promise<T>;
}

export async function runRentalInvoiceApiRoute<T>(
  options: RentalInvoiceApiRouteOptions<T>,
