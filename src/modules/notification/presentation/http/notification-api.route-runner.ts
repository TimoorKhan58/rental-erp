...
export interface NotificationApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => NotificationApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: NotificationApplicationServices,
  ) => Promise<T>;
}

export async function runNotificationApiRoute<T>(
  options: NotificationApiRouteOptions<T>,
