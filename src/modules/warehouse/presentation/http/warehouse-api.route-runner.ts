...
import type { WarehouseApplicationServices } from "@/modules/warehouse/application/services/warehouse-application-services.interface";
...
export interface WarehouseApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => WarehouseApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: WarehouseApplicationServices,
  ) => Promise<T>;
}

export async function runWarehouseApiRoute<T>(
  options: WarehouseApiRouteOptions<T>,
