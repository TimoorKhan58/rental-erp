...
export interface RepairApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => RepairApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: RepairApplicationServices,
  ) => Promise<T>;
}

export async function runRepairApiRoute<T>(
  options: RepairApiRouteOptions<T>,
