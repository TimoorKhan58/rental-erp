...
export interface ExternalRentalApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => ExternalRentalApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: ExternalRentalApplicationServices,
  ) => Promise<T>;
}

export async function runExternalRentalApiRoute<T>(
  options: ExternalRentalApiRouteOptions<T>,
