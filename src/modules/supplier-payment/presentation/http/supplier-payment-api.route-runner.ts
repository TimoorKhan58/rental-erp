...
import type { SupplierPaymentApplicationServices } from "@/modules/supplier-payment/application/services/supplier-payment-application-services.interface";
...
export interface SupplierPaymentApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => SupplierPaymentApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: SupplierPaymentApplicationServices,
  ) => Promise<T>;
}

export async function runSupplierPaymentApiRoute<T>(
  options: SupplierPaymentApiRouteOptions<T>,
