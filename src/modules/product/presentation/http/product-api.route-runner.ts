...
import type { ProductApplicationServices } from "@/modules/product/application/services/product-application-services.interface";
import {
...
export interface ProductApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => ProductApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: ProductApplicationServices,
  ) => Promise<T>;
}

export async function runProductApiRoute<T>(
  options: ProductApiRouteOptions<T>,
