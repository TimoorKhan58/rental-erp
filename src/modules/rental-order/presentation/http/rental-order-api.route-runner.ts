...
export interface RentalOrderApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => RentalOrderApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: RentalOrderApplicationServices,
  ) => Promise<T>;
}

export async function runRentalOrderApiRoute<T>(
  options: RentalOrderApiRouteOptions<T>,
