...
export interface ReturnApiRouteOptions<T> {
  request: NextRequest;
  route: string;
  httpMethod: string;
  permission: Permission;
  resolveServices: (ctx: ExecutionContext) => ReturnApplicationServices;
  handler: (
    ctx: ExecutionContext,
    services: ReturnApplicationServices,
  ) => Promise<T>;
}

export async function runReturnApiRoute<T>(
  options: ReturnApiRouteOptions<T>,
